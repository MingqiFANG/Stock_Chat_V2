...
def _retry_numeric_literal_query(expr: str) -> str:
    """把 '20250101' 这类纯数字字符串与比较运算改成不带引号，缓解 int64 vs str 的比较错误。"""
    expr2 = re.sub(r"(==|>=|<=|>|<)\s*'(\d+)'", r"\1 \2", expr)
    expr2 = re.sub(r"(==|>=|<=|>|<)\s*\"(\d+)\"", r"\1 \2", expr2)
    return expr2

def _flatten_cols_any(columns) -> List[str]:
    """
    安全拍平任意层数的列索引：
    - 单层：转成字符串列表
    - 多层：把元组各层用下划线连接（忽略 None/空字符串）
    """
...
def df_groupby_agg(
    ...
    # ✅ 稳健拍平（支持任意层数的 MultiIndex）
    out.columns = _flatten_cols_any(out.columns)
    out = out.reset_index()
